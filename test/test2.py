...
class ImageBoundingBoxApp:
            ...
            if image_path in self.bounding_boxes:
                for bbox in self.bounding_boxes[image_path]:
                    ...
                    self.canvas.create_rectangle(
                        ...
                        bbox["coords"][2] * self.resize_ratio, bbox["coords"][3] * self.resize_ratio,
                        outline="red", tag="bbox")
                    # Display label near the bounding box
                    self.canvas.create_text(
                        bbox["coords"][0] * self.resize_ratio,
    ...
    def on_done(self):
        ...
        # Iterate over image files to gather unique labels
        for image_path in self.image_files:
            if image_path in self.bounding_boxes:
                for bbox in self.bounding_boxes[image_path]:
                    class_names.add(bbox["label"])

        # Sort class names and create a mapping of class names to indices
        ...
        for image_path in self.image_files:
            ...
            shutil.copy(image_path, dataset_path)
